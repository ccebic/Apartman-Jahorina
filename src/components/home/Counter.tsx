...
export const CounterUp = () => {
   ...
   return (
      <div className="counter p-7 lg:p-10 relative" ref={counterSectionRef}>
         <div className="absolute w-full h-10 bg-no-repeat left-0 rotate-180 -bottom-4" style={CurveStyle}></div>
         <div className="container mx-auto">
            <div
               id="content-counter"
               className="content flex flex-col gap-5 items-center md:grid md:grid-cols-2 md:justify-items-center justify-center lg:flex lg:flex-row"
            >
               <CounterBox icon={faUser} units="" text="Happy guests">
                  {counterOn && <CountUp start={0} end={400} duration={4.2}></CountUp>}
               </CounterBox>
               <CounterBox icon={faPersonSkiing} units="km" text="Length of ski trail">
                  {counterOn && <CountUp start={0} end={45} duration={4.2}></CountUp>}
               </CounterBox>
               <CounterBox icon={faMountain} units="m" text="Altitude">
                  {counterOn && <CountUp start={0} end={1350} duration={4.2}></CountUp>}
               </CounterBox>
               <CounterBox icon={faStar} units="" text="Five stars">
                  {counterOn && <CountUp start={0} end={340} duration={4.2}></CountUp>}
               </CounterBox>
            </div>
         </div>
      </div>
